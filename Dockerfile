...
RUN --mount=type=cache,target=/opt/ccache \
    ...
    MAX_JOBS=32 TORCH_CUDA_ARCH_LIST="7.0 7.2 7.5 8.0 8.6 8.7 8.9 9.0 9.0a" TORCH_NVCC_FLAGS="-Xfatbin -compress-all" \
    CMAKE_PREFIX_PATH="$(dirname $(which conda))/../" \
    python setup.py install

FROM conda as conda-installs
ARG PYTHON_VERSION=3.11
ARG INSTALL_PATH=cu121
...
RUN case ${TARGETPLATFORM} in \
         ...
         *)              pip install --index-url https://download.pytorch.org/${INSTALL_CHANNEL}/${INSTALL_PATH#.}/ torch torchvision torchaudio ;; \
    esac && \
    /opt/conda/bin/conda clean -ya
